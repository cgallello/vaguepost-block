...
4. In Chrome **Settings → AI in Chrome**, turn on **On-device AI**. This per-user setting must be on before Chrome will download its local generative model.
5. Some Chrome builds expose a **Prompt API** flag at `chrome://flags/#prompt-api`; enable it when present. Older Chrome builds may additionally expose `chrome://flags/#optimization-guide-on-device-model` and `chrome://flags/#prompt-api-for-gemini-nano`; enable those when present, then relaunch Chrome. Automatic classification remains disabled until a supported model is available. Chrome also requires a supported desktop OS, at least 22 GB free on the volume containing the Chrome profile, and either at least 16 GB RAM plus four CPU cores or a GPU with more than 4 GB VRAM. Check `chrome://on-device-internals` for the model's exact download/error state. If it says **Pending Assets**, keep Chrome open while the one-time download completes, then retry **Prepare local AI** until VagueBlock reports **Local AI ready**.
