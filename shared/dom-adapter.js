(() => {
  function followStateFromLabels(labels, handle = "") {
  ...
  function extractQuoteCandidate(article) {
    if (!article) return null;
    const links = [...article.querySelectorAll('a[href*="/status/"]')];
    const ids = [...new Set(links.map((link) => {
      const rawHref = link.getAttribute('href') || link.href || '';
      try { return new URL(rawHref, location.href).pathname.match(/\/status\/(\d+)$/)?.[1] || ''; } catch { return rawHref.match(/\/status\/(\d+)$/)?.[1] || ''; }
    }).filter(Boolean))];
    if (ids.length < 1) return null;
    // Prefer X's stable author wrapper. Newer conversation views sometimes
    // omit it, so use only a profile link that appears before the first
    // status link; quoted-account links occur after the author's post link.
    const authorLink = article.querySelector('[data-testid="User-Name"] a[href^="/"]') || (() => {
      ...
      const profileLinks = [...article.querySelectorAll('a')].filter((link) => {
        const rawHref = link.getAttribute('href') || link.href || '';
        let path = rawHref;
        try { path = new URL(rawHref, location.href).pathname; } catch { /* malformed links are not author evidence */ }
    ...
    const handle = (() => {
      ...
      try {
        return new URL(rawHref, location.href).pathname.split('/').filter(Boolean)[0] || '';
      } catch {
  ...
  globalThis.VGBDom = { followStateFromLabels, extractQuoteCandidate };
})();
