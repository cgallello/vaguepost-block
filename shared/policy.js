(() => {
  ...
  function confidenceGate(result, sensitivity = "standard") { return Boolean(result && result.isVague === true && vagueReason(result) && result.concreteSubjectPresent !== true && Number.isFinite(result.confidence) && result.confidence >= (CONFIDENCE_THRESHOLDS[sensitivity] ?? CONFIDENCE_THRESHOLDS.standard)); }
  function automaticBlockAllowed(result) { return Boolean(result?.isVague === true && vagueReason(result) && result.concreteSubjectPresent !== true && Number.isFinite(result.confidence) && result.confidence >= 0.90); }
